...
def update_chat_log(username, chat_data):
    with session as s:
        s.execute(sqlalchemy.text("UPDATE users SET data = :data WHERE username = :username"),
...
def get_chat_log(username):
    with session as s:
        result = s.execute(sqlalchemy.text("SELECT data FROM users WHERE username = :username"),
                           {"username": username})
